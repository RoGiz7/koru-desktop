...
export const CHANGELOG: ChangelogEntry[] = [
  ...
  },
  {
    ...
    es: [
      ...
      "⚔️ La ficha de un hostil ahora empieza por lo único que importa cuando lo tienes a un salto: si te ha matado, si tú le has matado, y si suele venir solo o acompañado. Todo sale de tus propios killmails, que llevaban guardados desde el primer día con el detalle de cada atacante dentro y no se leían para nadie que no fueras tú: sus naves, su daño y con quién iba. Va arriba del todo y en una línea, porque eso no se lee con calma. Con dos avisos escritos en la propia pantalla: son las naves que TÚ le has visto usar, no todas las suyas — lo que haya hecho sin ti delante no aparece; y si nunca os habéis cruzado te lo dice con palabras, «nunca te has cruzado con él», en vez de enseñarte un marcador a cero que se lee como que es inofensivo.",
      "🕰️ Y «hace 10222h» dejó de existir. Los tiempos se paraban en horas, así que catorce meses se escribían como un número de cinco cifras que nadie lee como «hace más de un año». Ahora suben solos de horas a días, semanas, meses y años.",
      ...
      "🛰️ El intel deja de fichar como hostiles cosas que no son gente. En una revisión de los datos reales aparecieron 4.981 nombres que EVE dice que no existen contados como pilotos: abreviaturas que escribís en mayúsculas («WH», «YPW», «MC»), letras sueltas, y trozos de nombres de nave. La solución no es una lista de palabras prohibidas más larga —«YPW» es la abreviatura de un sistema de tu región y mañana es otra— sino que Koru use lo que ya sabía: cada nombre raro se pregunta UNA vez, se apunta la respuesta, y si EVE dice que no es de nadie deja de proponerse para siempre. Se afina solo mientras lo usas. Y por longitud: EVE no permite nombres de una o dos letras, así que «I» o «MC» no pueden ser un piloto — comprobado antes contra los nombres reales de la propia base de datos para no partir a los que llevan partículas cortas, como «Bedwin Al Ishira».",
      "🇨🇳 Y las naves cantadas en otro idioma ya se ven. Si en tu canal hay gente con el cliente en chino, cuando cantaban un Sabre la línea llegaba sin nave: Koru solo conocía los nombres en inglés, así que se le caía por el camino. Un interdictor invisible, que es justo la nave que te impide saltar. Ahora el catálogo trae los nombres de nave en los siete idiomas del juego —1.327 en total, sacados de los datos oficiales— y no se acepta ninguno que pudiera confundirse con un sistema o con otra nave. Sin ningún ajuste que configurar: un canal de intel no tiene idioma, lo tienen las personas que escriben en él, así que se entienden todos a la vez.",
    ...
    en: [
      ...
      "⚔️ A hostile's profile now opens with the only thing that matters when he's one jump away: whether he's killed you, whether you've killed him, and whether he tends to come alone or with company. It all comes from your own killmails, which had been stored since day one with every attacker's detail inside and were never read for anyone but you: his ships, his damage and who he was with. It goes at the very top and in one line, because that isn't read at leisure. With two warnings written on the screen itself: these are the ships YOU have seen him fly, not all of his — anything he did without you there doesn't show; and if you've never met, it says so in words, «you've never crossed paths with him», instead of showing you a zeroed scoreboard that reads as harmless.",
      "🕰️ And «10222h ago» is gone. Times stopped at hours, so fourteen months were written as a five-digit number nobody reads as «over a year ago». They now step up on their own from hours to days, weeks, months and years.",
      "🔢 And the Hunter now explains why there are two numbers. The list said «×232» while the same pilot profile said «155 sightings», and both read the same: «how many times have I seen him». They are two different questions and both are true: how many times he was NAMED in intel, and how many distinct sightings come out of that, which are the ones carrying a system and a time and feeding his trail. They now appear together, labelled, with a line explaining it when they differ.",
      ...
      "🛰️ Intel stops filing things that aren't people as hostiles. A pass over the real data turned up 4.981 names EVE says don't exist being counted as pilots: abbreviations people type in caps («WH», «YPW», «MC»), single letters, and fragments of ship names. The answer isn't a longer list of banned words —«YPW» is the shorthand for a system in your region and tomorrow it's another— but Koru using what it already knew: each odd name is asked ONCE, the answer is kept, and if EVE says it belongs to nobody it stops being suggested for good. It sharpens itself as you use it. And by length: EVE doesn't allow one- or two-letter names, so «I» or «MC» can't be a pilot — checked first against the real names in your own database so it wouldn't break the ones with short particles, like «Bedwin Al Ishira».",
      "🇨🇳 And ships called out in another language now show up. If someone in your channel runs the client in Chinese, a Sabre they reported arrived with no ship at all: Koru only knew the English names, so it fell through. An invisible interdictor — the very ship that stops you jumping. The catalogue now carries ship names in all seven of the game's languages —1,327 of them, taken from the official data— and none is accepted if it could be mistaken for a system or another ship. With nothing to configure: an intel channel doesn't have a language, the people writing in it do, so all of them are understood at once.",
      ...
      "🖼️ The ship art behind each section no longer looks like a pasted cutout. It showed as a rectangle with a hard edge, and that wasn’t a matter of taste: the image EVE serves is square and carries its own background, the box we put it in was shorter, and the gradient meant to fade it was measured against a different size than the one being cropped — so it never reached the edge. The fade now closes before touching any edge, and the ship takes up a good deal more width.",
  ...
  },
  {
    ...
    es: [
      ...
      "🐛 Ratas que salían en español (una sola línea invertida en 6,6 GB envenenaba el diccionario; ahora manda el catálogo oficial: 6.192 NPC), «Veldspar*» como mena fantasma y los nombres de sistema acabados en asterisco partiendo su sistema en dos — los tres se corrigen solos, sin reescanear.",
    ...
    en: [
      ...
      "🐛 Rat names showing in Spanish (a single reversed line in 6.6 GB poisoned the dictionary; the official catalogue now rules: 6,192 NPCs), “Veldspar*” as a ghost ore, and system names ending in an asterisk splitting their system in two — all three self-heal, no re-scan needed.",
