...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    date: "2026-09-02",
    es: [
      "📜 Tus contratos, por fin visibles — y con memoria más larga que la del juego. Koru llevaba meses guardando cada contrato en cada sincronización, pero no había ninguna pantalla que los enseñara. Ahora hay una, en Comercio: quién lo emitió, quién lo aceptó, volumen, ISK, ISK/m³ y el tiempo real de entrega cuando lo hubo. Lo importante es lo que dice arriba del todo: desde qué día tiene libro. EVE solo te deja consultar los últimos 30 días, así que todo lo anterior no es que no pasara — es que nadie lo estaba guardando. Desde ahora tu histórico crece por encima de lo que el juego recuerda.",
      "📄 Y una nota puede esperar a que alguien te complete un contrato. Añade a un piloto como tarea de una nota —«el carbono se lo dejé a Reclutador»— y esa tarea se tacha sola cuando esa persona te complete un contrato, quedando marcada como cerrada por contrato y no a mano. Con el matiz dicho en la propia tarjeta: Koru ve que el contrato se completó, no lo que llevaba dentro.",
      "🎖️ Siete medallas nuevas para quien manda. El dominio de Mando de la Bitácora pasa de cuatro a once, y todas salen de lo que ya grabas cuando llevas una flota: los sistemas que recorristeis en una sola op, la op más larga, cuántas alas y escuadras llegaste a tener, los cambios de nave de los demás, las ops en las que nadie se quedó descolgado, las semanas seguidas saliendo, y la que más nos gusta — cuánta gente de fuera ha repetido op contigo. Esa última no cuenta a tus propios personajes a propósito: si volaras con tus alts, «repetirían» siempre y la medalla no diría nada de ti como comandante. Y una que estuvo sobre la mesa se quedó fuera: premiaba las ops sin pérdidas, pero Koru solo ve los killmails de tus personajes — habría sido una medalla por no mirar.",
      "🧹 Quitar un personaje ahora son dos decisiones, y la segunda borra de verdad. Antes había un solo botón, «Cerrar sesión», que hacía las dos cosas a la vez y hacía mal la segunda: se llevaba una parte de tus datos y dejaba el resto —assets, contratos, industria, planetas, flotas, registros de combate— dentro de Koru. Ahora primero DESCONECTAS: se retira el permiso, Koru deja de pedirle nada a EVE y su histórico se queda entero por si lo quieres seguir mirando. Y solo después, si quieres, borras sus datos: con la copia de seguridad ofrecida ahí mismo, diciéndote antes qué se lleva y qué no, y enseñándote al terminar la lista exacta de lo borrado. Volver a dar acceso lo reactiva sin perder nada.",
      ...
      "🗣 Los avisos del sistema ya hablan tu idioma. Los que salen fuera de la ventana —extractores de planetas parados, un logro nuevo— estaban escritos en castellano para todo el mundo, tuvieras Koru en inglés o no: la parte de Koru que los manda no tenía forma de saber qué idioma habías elegido. Ahora lo sabe, y siguen al selector como el resto de la aplicación.",
      "🧪 Y un botón para probarlos, en Ajustes → Empezar. Si alguna vez no te llega un aviso —los del intel salen por el mismo sitio—, este botón manda uno de prueba y te dice cuál de las dos cosas pasa: que el sistema lo rechace, o que lo esté silenciando Windows. Son dos averías distintas con dos arreglos distintos, y hasta ahora no había manera de distinguirlas.",
    ],
    en: [
      "📜 Your contracts, visible at last — and with a longer memory than the game's. Koru had been storing every contract on every sync for months, but no screen ever showed them. Now there is one, in Trading: who issued it, who accepted it, volume, ISK, ISK/m³ and the real delivery time when there was one. The important part is the line at the top: the date its ledger starts. EVE only lets you query the last 30 days, so anything older didn't fail to happen — nobody was saving it. From now on your history grows past what the game remembers.",
      "📄 And a note can wait for someone to complete a contract for you. Add a pilot as a note's task —«I lent the carbon to Reclutador»— and that task ticks itself when that person completes a contract for you, marked as closed by contract rather than by hand. With the caveat stated on the card itself: Koru sees that the contract was completed, not what was inside it.",
      "🎖️ Seven new medals for whoever commands. The Log's Command domain goes from four to eleven, and they all come out of what you already record when you run a fleet: the systems you covered in a single op, your longest op, how many wings and squads you got up to, everyone else's ship changes, the ops where nobody got left behind, consecutive weeks flying, and our favourite — how many people from outside have flown a second op with you. That last one deliberately ignores your own characters: if you flew with your alts they would «come back» every time, and the medal would say nothing about you as a commander. And one that was on the table got dropped: it rewarded ops with no losses, but Koru only sees killmails for your own characters — it would have been a medal for not looking.",
      "🧹 Removing a character is now two decisions, and the second one really deletes. There used to be a single «Log out» button that did both things at once and got the second one wrong: it took part of your data and left the rest —assets, contracts, industry, planets, fleets, combat logs— inside Koru. Now you DISCONNECT first: access is withdrawn, Koru stops asking EVE for anything about them, and their history stays intact in case you still want to look at it. Only then, if you want to, you delete their data: with a backup offered right there, telling you beforehand what goes and what doesn't, and showing you the exact list of what was deleted when it finishes. Granting access again reactivates them without losing anything.",
      ...
      "🗣 System notifications now speak your language. The ones that appear outside the window —stopped planetary extractors, a new achievement— were written in Spanish for everyone, whether you had Koru in English or not: the part of Koru that sends them had no way of knowing which language you'd picked. Now it does, and they follow the selector like the rest of the app.",
      "🧪 And a button to test them, in Settings → Get started. If a notification ever doesn't reach you —intel alerts come out the same way— this button sends a test one and tells you which of two things is happening: the system refusing it, or Windows silencing it. Those are two different faults with two different fixes, and until now there was no way to tell them apart.",
    ],
  },
  {
    version: "0.48.0",
    ...
    es: [
      ...
      "📝 Las tareas de una nota se tachan solas cuando llega el objeto. Apuntas los siete materiales que te faltan para un proyecto y Koru los va marcando según aparecen en tu hangar, con cantidades: si pides cinco y llegan tres, te avisa «3/5» y NO la da por hecha. Mira el hangar y no la llegada, así que da igual si vienen de un viaje o de tres, y sigue contando bien después de reiniciar. Y una corrección importante: una nota CON TAREAS ya nunca se cierra sola — un proyecto termina cuando terminan sus tareas, no cuando llega el primer objeto.",
      "🧭 Una guía de inicio, para saber qué hay y para qué sirve sin preguntarle a nadie. Tres pestañas: por dónde empezar, qué puedes hacer y qué mirar si algo no va. Cada respuesta termina en un botón que te lleva —el del intel abre el mapa con la capa puesta, los de problemas abren el ajuste exacto—, porque «ve a Ajustes → Intel» te obliga a leer, recordar y navegar, y un botón ya lo ha resuelto. Sale sola la primera vez y se abre cuando quieras desde Ajustes → Empezar.",
      "💡 Pistas donde de verdad hacen falta. Una sola por sección, pegada a lo que señala: en Industria, que los costes no cuadran si no declaras cómo es cada estructura —ESI no da los bonuses reales—; en Assets, que el fit de una nave se abre desde el icono de la columna «Contenedor» y no pinchando la nave; en Notas, que una nota puede avisarte sola. Cada una se calla para siempre con su ×, y se apagan todas desde Ajustes.",
      "📦 Assets responde mucho antes, y viene ordenado de otra forma. Entrar en la sección recién abierto Koru se hacía esperar, y no porque la sección fuera lenta: la sincronización de arranque le estaba quitando el sitio. Ahora espera un poco antes de ponerse, así que lo primero que abras entra suelto — cuánto se note dependerá de lo que tengas dentro. El top por valor pasa a TRES columnas —Naves, Materiales e Items, cinco de cada una— porque un top único mezclaba una nave con un montón de mineral y no dejaba comparar nada. Y los blueprints se quedan fuera: su precio de mercado no es lo que sacarías por ellos.",
      "🔢 Cada categoría dice cuánto tiene, en su propia pestaña. Se ha retirado la gráfica de distribución: medía unidades, y con cien millones de minerales el resto de barras eran un muñón. El número está ahora donde se decide —al elegir por dónde filtrar— y es exactamente el número de entradas que verás al pulsar.",
      "🎓 Koru empieza a llevar la cuenta de las HABILIDADES que entrenas. Desde esta versión mira tus skills en cada sincronización y apunta cada nivel nuevo que consigues, con fecha EXACTA cuando la cola de entrenamiento lo tenía prometido. Todavía no hay pantalla que lo enseñe: es el cimiento de las medallas de estudios, y necesita tiempo para tener algo que contar. Las habilidades que ya tenías no se pueden fechar —EVE no guarda ese historial en ningún sitio al que se pueda preguntar—, así que tu registro de estudios empieza el día que instales esta versión.",
      "🐛 Correcciones, varias señaladas por vosotros. Entre ellas, un error en cómo estaba expresado lo de los registros de chat en Social: SÍ se pueden leer —son texto plano en tu disco—, y lo que hace Koru es coserlos, porque el cliente los parte en cientos de ficheros y no te enseña el historial. Gracias por decirlo. También: los iconos de los blueprints salían rotos en Assets —y cada uno gastaba una petición que el servidor rechazaba— y siete categorías se quedaban en castellano con la aplicación en inglés.",
    ],
    en: [
      ...
      "📝 A note's tasks tick themselves off when the item arrives. Write down the seven materials you're missing for a project and Koru marks them as they show up in your hangar, with quantities: ask for five and get three, and it tells you «3/5» and does NOT call it done. It looks at the hangar, not at the delivery, so it doesn't matter whether they come in one trip or three, and it still counts right after a restart. And an important correction: a note WITH TASKS never closes itself any more — a project ends when its tasks end, not when the first item turns up.",
      "🧭 A getting-started guide, so you can find out what's there and what it's for without asking anyone. Three tabs: where to start, what you can do, and what to check if something isn't working. Every answer ends in a button that takes you there —the intel one opens the map with the layer already on, the troubleshooting ones open the exact setting— because «go to Settings → Intel» makes you read, remember and navigate, and a button has already done it. It shows up on its own the first time, and you can open it whenever you like from Settings → Get started.",
      "💡 Hints where they're actually needed. One per section, sitting next to what it points at: in Industry, that your costs won't add up unless you declare what each structure is like —ESI doesn't give an installation's real bonuses—; in Assets, that a ship's fit opens from the icon in the «Container» column and not by clicking the ship; in Notes, that a note can warn you on its own. Each one is silenced for good with its ×, and you can turn them all off from Settings.",
      "📦 Assets responds much sooner, and comes laid out differently. Going into the section right after opening Koru used to keep you waiting, and not because the section was slow: the start-up sync was taking its place in the queue. It now holds off for a moment, so whatever you open first gets a clear run — how much you notice will depend on what you've got in there. The top-by-value now comes in THREE columns —Ships, Materials and Items, five of each— because a single top mixed a ship in with a pile of ore and let you compare nothing. And blueprints are left out: their market price isn't what you'd actually get for them.",
      "🔢 Every category says how much it holds, right on its own tab. The distribution chart is gone: it measured units, and with a hundred million minerals the other bars were a stub. The number now lives where the decision is made —as you pick what to filter by— and it's exactly how many entries you'll see when you click.",
      "🎓 Koru starts keeping track of the SKILLS you train. From this version it looks at your skills on every sync and records each new level you reach, with an EXACT date whenever the training queue had promised it. There's no screen for it yet: it's the foundation the study medals will stand on, and it needs time before it has anything to say. The skills you already had can't be dated —EVE keeps no such history anywhere you can ask— so your study record starts the day you install this version.",
      "🐛 Fixes, several of them pointed out by you. Among them, a mistake in how the chat logs were described in Social: they CAN be read —they're plain text on your disk— and what Koru does is stitch them together, because the client splits them across hundreds of files and never shows you the history. Thanks for saying so. Also: blueprint icons were showing up broken in Assets —and each one spent a request the server rejected— and seven categories stayed in Spanish with the app set to English.",
    ],
  },
  {
    version: "0.47.0",
